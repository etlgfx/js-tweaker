...
function config(cb) {
    if (_configCache !== undefined) {
        ...
        try {
            _options.fileList.forEach(function (f) {
...
config.options = function (options) {
	...
	_options.fileList = _options.fileList.map(function (f) {
		return f.replace(/\{ENV\}/, _options.nodeEnv);
	});
